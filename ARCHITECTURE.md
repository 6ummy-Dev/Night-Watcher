...
| Belt partials | `segButtons`/`segmented` and the option tables, `formatSwitch`/`tierSwitch`/`scopeSwitch`/`themeRow`, `paperRow` (Home's one door to Nocturne, 6.2.1), `includeBlock`, `buckleLines`, `masterChooser`, `introStats`/`introBlock`, `pendingBanner`. |
| Views | `heroHead` (the shared hero top: kick, meta line, title, badges, blurb, rule) and `scoreboard`/`scoreTile`, `viewHome`, `viewNext` (with `uptoButton`, `parkedRow`, `watchNotes`), `viewWatch` (with `chipSet`, `filmRow`, `groupBlock`, `scopeNote`, `emptyBlock`, the group helpers `gDone`/`gSkip`/`gSub`/`gBarFill`), the skyline (`ROOFS`, `roofOf`, `crownState`, `skyline`), `drawShareCard`/`shareCardBlock`/`cardFile`/`lineOf`, `viewStats` (with `progFold`/`progRows`, `nightsOf`, `doneBy`/`doneByLine`, `nightsLine`, `favList`/`favText`/`favBlock`, `installBlock`). |
...
| Belt open / drop / close | `openBelt`, `beltDropOpen`, `closeBelt(via)`, the drop-scroll arming (`armDropScroll`/`disarmDropScroll`/`dropScrollOnce` — the one scroll the drop is allowed), `beltWatch` (the two observers), `scrubBelt`, `parkFocus`, and the two focus hand-offs (6.1.0) — `dropFocus` (the peek's doors land on the pressed path inside the dropped belt) and `beltFocus` (after Escape, a focus the closing belt took with it goes back to the peek or the strip). |
...
| Delegated events | `#tabs` click; `#topBtn`, `#ringBtn`, `#markBtn`; `#beltpeek` click (a native button, so Enter and Space arrive as the click) and the Escape handler; the one `#view` click handler (every `data-*` action); `#view` change (file import) and input (search, 180 ms). |
| Standalone detection | `IOSDEVICE`, `isStandalone`. The installed frame has no script (guard 162): the 4.0.8/4.0.9 viewport heal and pad reclaim were retired in 6.0.9 with `black-translucent`, and 6.1.2's rotation reseat was removed in 6.1.3 — the flip is iOS 27's (NOTES). The installed footer is a CSS rule (guard 64). |
