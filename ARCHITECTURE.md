...
| Standalone detection | `IOSDEVICE`, `isStandalone` (the 4.0.8/4.0.9 viewport heal and pad reclaim were retired in 6.0.9 with `black-translucent`); the installed frame readout (6.1.1) — `frameWatch` (observers on `#fprobe` and the header, installed only), `frameNote` (keeps the at-launch and current readings), `frameText` (the line under Progress's Build line). |
