# Changelog — the archive (1.0.0 to 3.9.7)

The 1.x, 2.x and 3.x entries of `CHANGELOG.md`, moved here whole in 4.9.0 so
the live file opens on the current line. Nothing was rewritten: the numbers
in these entries are the numbers they stated when they shipped, and a later
entry that corrects one says so in its own text. The guards read only the
live file's newest heading and its dates, so this archive is a record and
nothing checks it.
