...
run_case "the head loses description" \
  ...
  "${P}a='<meta name=\"description\" content=\"Every Batman movie and series in order — 137 films and 71 seasons of TV across 44 continuities — no spoilers. By universe, by the arc of one life, or by release.\">';assert a in s;s=s.replace(a,'',1);${W}" \
  guards "" 153
...
run_case "the head loses og:description" \
  ...
  "${P}a='<meta property=\"og:description\" content=\"Every Batman movie and series in order — 137 films and 71 seasons of TV across 44 continuities — no spoilers. Choose a path once and track what you have seen.\">';assert a in s;s=s.replace(a,'',1);${W}" \
  guards "" 153
...
run_case "the head loses og:image:alt" \
  ...
  "${P}a='<meta property=\"og:image:alt\" content=\"Night Watcher — 137 films, 71 seasons, 44 continuities. Batman watch orders. No spoilers.\">';assert a in s;s=s.replace(a,'',1);${W}" \
  guards "" 153
...
run_case "the README drops its row for nocturne/" \
  "README's file table does not list: nocturne/" \
  "import io,re;p='README.md';s=io.open(p,encoding='utf-8').read();a=re.search(r'^\| \`'+re.escape('nocturne/')+r'\` \|.*\n',s,re.M).group(0);s=s.replace(a,'',1);io.open(p,'w',encoding='utf-8').write(s)" \
  guards "" 45

run_case "the README drops its row for docs/nocturne/" \
  "README's file table does not list: docs/nocturne/" \
  "import io,re;p='README.md';s=io.open(p,encoding='utf-8').read();a=re.search(r'^\| \`'+re.escape('docs/nocturne/')+r'\` \|.*\n',s,re.M).group(0);s=s.replace(a,'',1);io.open(p,'w',encoding='utf-8').write(s)" \
  guards "" 45

run_case "the README drops its row for qa/nocturne.js" \
  "README's file table does not list: qa/nocturne.js" \
  "import io,re;p='README.md';s=io.open(p,encoding='utf-8').read();a=re.search(r'^\| \`'+re.escape('qa/nocturne.js')+r'\` \|.*\n',s,re.M).group(0);s=s.replace(a,'',1);io.open(p,'w',encoding='utf-8').write(s)" \
  guards "" 45

run_case "the README drops its row for qa/nocturne-fixture/" \
  "README's file table does not list: qa/nocturne-fixture/" \
  "import io,re;p='README.md';s=io.open(p,encoding='utf-8').read();a=re.search(r'^\| \`'+re.escape('qa/nocturne-fixture/')+r'\` \|.*\n',s,re.M).group(0);s=s.replace(a,'',1);io.open(p,'w',encoding='utf-8').write(s)" \
  guards "" 45

run_case "the README drops its row for .github/workflows/nocturne-fence.yml" \
  "README's file table does not list: .github/workflows/nocturne-fence.yml" \
  "import io,re;p='README.md';s=io.open(p,encoding='utf-8').read();a=re.search(r'^\| \`'+re.escape('.github/workflows/nocturne-fence.yml')+r'\` \|.*\n',s,re.M).group(0);s=s.replace(a,'',1);io.open(p,'w',encoding='utf-8').write(s)" \
  guards "" 45

run_case "the README drops its row for .github/CODEOWNERS" \
  "README's file table does not list: .github/CODEOWNERS" \
  "import io,re;p='README.md';s=io.open(p,encoding='utf-8').read();a=re.search(r'^\| \`'+re.escape('.github/CODEOWNERS')+r'\` \|.*\n',s,re.M).group(0);s=s.replace(a,'',1);io.open(p,'w',encoding='utf-8').write(s)" \
  guards "" 45

run_case "the README drops its row for qa/nocturne-fonts/" \
  "README's file table does not list: qa/nocturne-fonts/" \
  "import io,re;p='README.md';s=io.open(p,encoding='utf-8').read();a=re.search(r'^\| \`'+re.escape('qa/nocturne-fonts/')+r'\` \|.*\n',s,re.M).group(0);s=s.replace(a,'',1);io.open(p,'w',encoding='utf-8').write(s)" \
  guards "" 45
...
run_case "the README drops its row for qa/split-ids.json" \
  "README's file table does not list: qa/split-ids.json" \
  "import io,re;p='README.md';s=io.open(p,encoding='utf-8').read();a=re.search(r'^\| \`'+re.escape('qa/split-ids.json')+r'\` \|.*\n',s,re.M).group(0);s=s.replace(a,'',1);io.open(p,'w',encoding='utf-8').write(s)" \
  guards "" 45
