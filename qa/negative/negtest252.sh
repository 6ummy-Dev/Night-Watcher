...
run_case "the name goes back to a fixed verb" \
  ...
  "${P}a='aria-label=\"0% — open progress\"';assert a in s;s=s.replace(a,'aria-label=\"Open progress\"',1);${W}"

run_case "the name keeps the words but loses the number" \
  "does not contain its visible text" \
  "${P}s=s.replace('aria-label=\"0% — open progress\"','aria-label=\"Progress — open progress\"',1);${W}"
