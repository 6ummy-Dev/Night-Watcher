...
/* INDEX
   ...
   LAYOUT
     ...
     156  Three small things that never leave the browser
...
var SECTIONS = null;
/* 5.3.1: the fixture corpus is read by one module — qa/negative/census.js —
   and cached for the run; sections 65, 113 and 138 and run-all.sh all ask
   it (see the module's header for why four readers were one too many). */
/* 5.3.1: THE @graph HAS ONE ORDER. Each JSON-LD blesser (95, 100) used to
   drop its node and push it last, so the order depended on which node had
   changed — and the order is worth 1.3 KB of gzip: the FAQPage's text is
   repeated by the seed ~36 KB downstream, inside gzip's 32 KB window only
   when the FAQPage is the graph's LAST node. Fixed order, FAQPage last. */
var GRAPH_ORDER = ["WebSite", "WebApplication", "ItemList", "FAQPage"];
function graphOrder(graph){
  return graph.slice().sort(function(a, b){
    var ia = GRAPH_ORDER.indexOf(a["@type"]), ib = GRAPH_ORDER.indexOf(b["@type"]);
...
/* EVERY <style> BLOCK, ONE WAY (4.9.0). Five sites extracted the CSS five
   ways, and two of them sliced from the first <style> to the first </style>
   — which stopped seeing the <noscript><style> the splash undo added in
   4.7.0, and would miss any second block. One reader: every block, comments
   stripped, joined; and the rule list off it. */
var CSS_TEXT = null, CSS_RULES = null;
function cssText(){
  if(CSS_TEXT === null){
    CSS_TEXT = (HTML.match(/<style>([\s\S]*?)<\/style>/g) || [])
      .map(function(b){ return b.replace(/^<style>|<\/style>$/g, ""); })
      .join("\n")
      .replace(/\/\*[\s\S]*?\*\//g, "");
  }
  return CSS_TEXT;
...
var FN_AST = null;      /* the parsed program, for the AST predicates below */
var FN_SCRIPT = "";     /* the concatenated script the AST was parsed from */
function fnIndex(){
...
/* AST PREDICATES (4.9.0). Most of this file pins exact source text with
   indexOf and regexes — deliberate, because the text IS the reviewed
   artefact, but a refactor that keeps the meaning and changes the spelling
   goes red in dozens of sections at once. The helpers below ask the parsed
   program questions a formatter cannot change the answer to: does this
   function call that one, what does this top-level var hold, which row of
   the SCHEMA table reads a key. Sections that pin behaviour rather than
   spelling go through these; sections pinning a specific reviewed string
   keep the string. With the parser unavailable every helper returns null
   and the section that asked says so, the same way optionalFn() does. */
...
function srcOf(node){ return node ? FN_SCRIPT.slice(node.start, node.end) : ""; }
function fnNode(name){
  if(!fnIndex()) return null;
  var hit = null;
...
/* The init expression of a top-level `var name = ...`, or null. */
function topVar(name){
  if(!fnIndex()) return null;
  var hit = null;
  FN_AST.body.forEach(function(n){
    if(n.type !== "VariableDeclaration") return;
    n.declarations.forEach(function(d){ if(d.id && d.id.name === name && d.init) hit = d.init; });
  });
  return hit;
}
function calleeName(c){
  ...
  if(c.type === "MemberExpression" && c.property && c.property.type === "Identifier"){
    return (c.object.type === "Identifier" ? c.object.name : "?") + "." + c.property.name;
  }
  return "";
}
/* Every call made inside a function (or any node): ["persist", "S.log.filter", ...]. */
function callsIn(node){
  var out = [];
  walk(node, function(n){ if(n.type === "CallExpression") out.push(calleeName(n.callee)); });
  return out;
}
function fnCalls(name, callee){
  var n = fnNode(name);
  return n ? callsIn(n).indexOf(callee) >= 0 : null;
}
/* A property of an object literal by key name, or null. */
function objProp(obj, key){
  if(!obj || obj.type !== "ObjectExpression") return null;
  var hit = null;
  obj.properties.forEach(function(p){
    var k = p.key && (p.key.name || p.key.value);
    if(k === key) hit = p.value;
  });
  return hit;
}
/* One row of the app's SCHEMA table, by key: the source of each of its
   fields, so a section can say "the log is read through dedupeLog" without
   spelling the whole row. null when the table or the row is missing. */
function schemaRow(key){
  var tbl = topVar("SCHEMA");
  if(!tbl || tbl.type !== "ArrayExpression") return null;
  var hit = null;
...
/* THE GIT INDEX, PARSED. Two sections read the tracked-file list — 144 (no
   .wrangler/ state in the index) and 45 (every tracked file is in the
   README's table) — and neither may shell out to git. Versions 2 and 3
   store whole NUL-terminated paths padded to 8 bytes; version 4
   prefix-compresses each path against the previous one (a varint "strip N"
   plus the suffix), which is why a byte scan for a literal path can miss
   it. Returns the path list, or null for anything it cannot parse; the
   callers say so and fall back. */
function gitIndexPaths(buf){
...
function trackedFiles(){
  ...
  if(!fs.existsSync(gidx)) return null;
  try{ return gitIndexPaths(fs.readFileSync(gidx)); }catch(e){ return null; }
}
function schemaKeys(){
  var tbl = topVar("SCHEMA"), out = [];
  if(!tbl || tbl.type !== "ArrayExpression") return out;
  tbl.elements.forEach(function(el){ var k = objProp(el, "k"); if(k) out.push(k.value); });
  return out;
}
...
if(BLESS){
  /* One retirement predates the surviving snapshot: the Scooby crossover
     ...
     skim warnings — recorded here as the known exception (2.5.0). A NEW
...
/* (2, continued) idHash is memoised, and the memo proves itself.
   NOT A SECTION OF ITS OWN, on the record (4.9.0): this block sat under a
   section-shaped header with no number, invisible to SECTION_HEADER, so its
   one failure printed as §2 and sections 107/138 credited §2 for it. A
   number of its own would mean renumbering every section after 2 and every
   fixture's sect argument, so it is filed where it prints — as the tail of
   section 2, whose subject (the frozen id set) is what the purity check
   walks. */
...
if(!manifest.icons.some(function(ic){ return /maskable/.test(ic.purpose || ""); })){
  fail("manifest has no maskable icon — Android will letterbox the install icon");
}
...
[["films", /\*\*(\d+) films/, actual.films],
 ...
 ["continuities", /(\d+) continuities/, actual.continuities]
  ...
  if(!m) return fail("README: could not find the " + t[0] + " count to verify — a count nobody can find is a count nobody checks");
...
(function(){
  ...
  if(!block){ fail("README: no Bruce\u2019s life bullet to check era names against"); return; }
...
[["meta description", /<meta name="description" content="([^"]+)"/],
 ...
 ["JSON-LD description", /"description":"([^"]*\d+ films[^"]*)"/]
  ...
  if(!m) return fail(t[0] + " is missing");
  var txt = m[1];
  [[/(\d+)\s+films/, actual.films, "films"],
   ...
   [/(\d+)\s+continuities/, actual.continuities, "continuities"]
    ...
    /* Silence used to read as success here: reword the sentence so a count no
       ...
       counts in prose. */
    if(!got) return fail(t[0] + " no longer states a " + c[2] + " count \u2014 " +
...
themes.forEach(function(t){
  ...
  pairs.forEach(function(pair){
    ...
    CONTRAST_ROWS.push([name, pair[0], pair[1], r]);
...
/* THE NUMBERS, ATTACHED — qa/contrast.md (4.9.2, the ISO triage's "the
   contrast numbers attached rather than claimed"). The sweep above measures
   every ink-on-surface pair on every run and the figures lived only in this
   run's notes; the evaluator had no repo checkout and could only take the
   claim on faith. The table is now a file: written from CONTRAST_ROWS under
   --bless, never typed, and on every other run the file must equal what
   this run would write — a palette change without a bless is a stale table,
   and a stale table fails like any other blessed artifact. The
   special-cased rules (.hero on the gradient, .lnk's blended fill, the
   UI-component exemptions) are asserted above and listed in the file's
   header as covered by assertion rather than tabulated. */
(function(){
  var floors = {};
  Object.keys(UI_EXEMPT).forEach(function(k){ floors[k] = 1; });
  var out = "# Night Watcher \u2014 measured contrast\n\n" +
  ...
  var byTheme = {};
  CONTRAST_ROWS.forEach(function(rw){ (byTheme[rw[0]] = byTheme[rw[0]] || []).push(rw); });
  Object.keys(byTheme).forEach(function(th){
    var rows = byTheme[th], surf = ["ink", "sunk", "card", "card2"];
    var byInk = {}, worst = null;
    rows.forEach(function(rw){
      (byInk[rw[1]] = byInk[rw[1]] || {})[rw[2]] = rw[3];
      if(!worst || rw[3] < worst[3]) worst = rw;
    });
    out += "\n## Theme: " + th + "\n\n| ink \\ surface | " +
           surf.map(function(s2){ return "--" + s2; }).join(" | ") + " |\n|" +
           new Array(surf.length + 2).join(" --- |") + "\n";
    Object.keys(byInk).sort().forEach(function(ink){
      out += "| --" + ink + (floors[ink] ? " (ui)" : "") + " | " +
        surf.map(function(s2){
          var r = byInk[ink][s2];
          return r === undefined ? "\u2014" : r.toFixed(2) + ":1";
        }).join(" | ") + " |\n";
    });
    out += "\nWorst pair: --" + worst[1] + " on --" + worst[2] + " at " +
           worst[3].toFixed(2) + ":1" + (floors[worst[1]] ? " (ui floor 3:1)" : "") + ".\n";
  });
  var ctPath = path.join(__dirname, "contrast.md");
  if(BLESS){
    fs.writeFileSync(ctPath, out);
    note("blessed qa/contrast.md \u2014 " + CONTRAST_ROWS.length + " measured pairs");
  } else if(!fs.existsSync(ctPath)){
    fail("qa/contrast.md is missing \u2014 the measured contrast table is a " +
         "blessed artifact; run: npm run bless");
  } else if(fs.readFileSync(ctPath, "utf8") !== out){
    fail("qa/contrast.md is stale \u2014 the palette or the ink sweep moved and " +
         "the attached numbers did not. Regenerate with: npm run bless");
  }
})();
...
/* The payload once carried `mode` beside `path` so a 1.1.0 build could read
   a newer save; 4.9.0 dropped the write (nobody is downgrading to 1.1.0) and
   kept the READ, because a save written before `path` existed still carries
   its ordering under `mode`. The path row reads the legacy key and never
   falls back to the live S.mode: that fallback once assigned a path to
   anyone who ticked something before choosing. */
(function(){
  var row = schemaRow("path");
  if(!row){ fail("SCHEMA has no path row — the chosen ordering would not be saved"); return; }
  if(schemaKeys().indexOf("mode") >= 0){
    fail("the payload writes `mode` again — it was a 1.1.0 downgrade mirror, " +
         "dropped in 4.9.0; the legacy key is read, never written");
  }
  if(!/isPath\(o\.mode\)/.test(row.read)){
    fail("restore() no longer reads the legacy `mode` key — a save written " +
         "before `path` existed would lose its ordering");
  }
  if(/S\.mode/.test(row.read)){
    fail("restore() falls back to S.mode when no path is chosen — a new user who " +
...
/* Two renderings of PATHS by design as of 1.5.0: the big blocks on first run
   (PATHS.map, the deck) and the segmented control once a path exists
   (segButtons("path", PATHS…) inside masterChooser). A third would mean a
   setting with three places to change it. */
var pathBlocks = (HTML.match(/PATHS\.map\(/g) || []).length +
                 (HTML.match(/segButtons\("path", PATHS/g) || []).length;
if(pathBlocks !== 2){
  fail("PATHS is rendered " + pathBlocks + " times, expected 2 " +
       "(the first-run deck and masterChooser)");
}
...
if(!/dedupeLog\(Array\.isArray\(v\)/.test((schemaRow("log") || {}).read || "")){
...
(function(){
  var row = schemaRow("groupOpen");
  if(!row){
    fail("groupOpen is not written to the saved payload \u2014 collapse state dies on reload");
    return;
  }
  if(!/flagsOf\(v, false\)/.test(row.read)){
    fail("restore() does not read the saved groupOpen back through flagsOf(v, false) " +
         "\u2014 collapse state would be written and never used");
  }
  /* Absent means open. Persisting anything but false would invert the default
     on the next build that adds a group. */
  if(!/v\[k\] === want/.test(fn("flagsOf"))){
...
if(!ogurl){
  fail("og:url is gone \u2014 a shared link would carry no canonical address of its own");
} else if(canon && canon[1] !== ogurl[1]){
...
/* 3.0.0 (planned as 2.8.0) made the three tiles controls. Each lands on the matching filter in
...
(function(){
  ...
  var PINNED = {
    "default-src":  "'none'",
    /* 'self' left in 4.9.0: there is no external stylesheet, so the source
       was granting nothing and reading as if one might arrive. The two
       inline blocks (<style> and the <noscript><style>) are what
...
(function(){
  ...
  /* The root half of the table was maintained by hand and drifted: LICENSE,
     ...
     and the hand list written to catch that drifted too (4.9.0: ten
     tracked files it never named, so deleting their rows passed). The
     tracked set now comes from the git index itself, where one exists; the
     hand list below is the floor for a tree without .git (a zip, a
     negative scratch copy), and is kept short on purpose. A row may name a
     directory with a trailing slash, which covers everything under it. */
  ...
  (tracked || FLOOR).forEach(function(f){
    if(f.indexOf("docs/") === 0 && onDisk.indexOf(f) >= 0) return;
    if(fs.existsSync(path.join(ROOT, f)) && onDisk.indexOf(f) < 0) onDisk.push(f);
  });
  var covered = function(f){
    if(listed[f]) return true;
    return Object.keys(listed).some(function(row){
      return row.slice(-1) === "/" && f.indexOf(row) === 0;
    });
  };
  var missing = onDisk.filter(function(f){ return !covered(f); });
  if(missing.length){
    fail("README's file table does not list: " + missing.join(", "));
  }
  note("file table: " + onDisk.length + " files checked, tracked set from " +
       (tracked ? "the git index" : "the floor list (no .git here)"));
...
if(!/oneOf\(\["anim", "live", "all"\], "anim"\)/.test((schemaRow("format") || {}).read || "")){
...
(function(){
  ...
  (function(){
    ...
    if(fs.existsSync(negDir)){
      /* A quoted label, so the run_case() definition at the top of every
         suite is not counted as one of its own fixtures (that was how the
         total came to be reported as 194 when it was 178); green_case counts
         too (3.7.1); leading whitespace is allowed (a fixture wrapped in an
         `if` runs). 5.3.1: all of that lives in ONE reader now —
         qa/negative/census.js — which this section, 113, 138 and run-all.sh
    ...
    SWEEP.forEach(function(src){
      counts.forEach(function(c){
        ...
        if(!seen && src[0] === "README.md" && c[3] !== "qa.yml"){
          fail("README: no " + c[0] + " count to check");
        }
      });
    });
  ...
  var POINTER_CARRIERS = ["docs/sw.js", "worker.js", "docs/_headers",
                          ...
                          /* 5.3.1: two more carriers said "NOTES.md" for stories
                             that live in NOTES-history.md, and this list did not
                             read them. */
                          ".github/workflows/qa.yml", "qa/browser-check.mjs"];
...
(function(){
  ...
  /* 5.3.1: THE SECOND URL. The sitemap lists llms.txt too, and its lastmod
     was two edits old twice (4.5.1 corrected the same drift once) because
     this section read only the first <lastmod>. A body has no date of its
     own to compare against, so the bless pattern holds it: qa/llms-txt.json
     records the file's hash and the date it was last stated to have
     changed. Same hash → the sitemap must still say that date; a different
     hash → llms.txt changed in this cut, the sitemap must say the newest
     CHANGELOG date, and `npm run bless` writes both the record and the
     sitemap line. */
  var LLMS_REC = path.join(__dirname, "llms-txt.json");
  var llmsPath = path.join(PUBLIC, "llms.txt");
  var llmsBlock = (sm.match(/<url>\s*<loc>[^<]*\/llms\.txt<\/loc>[\s\S]*?<\/url>/) || [""])[0];
  var llmsLast = (llmsBlock.match(/<lastmod>(\d{4}-\d{2}-\d{2})<\/lastmod>/) || [])[1];
  if(!fs.existsSync(llmsPath) || !llmsBlock || !llmsLast){
    fail("the sitemap no longer dates llms.txt — the second URL must carry a <lastmod>, or a crawler is told nothing about the one file written for it");
  } else {
    ...
    if(llmsLast !== want){
      if(BLESS){
        ...
        fs.writeFileSync(path.join(PUBLIC, "sitemap.xml"), sm2);
        note("rewrote the sitemap's llms.txt lastmod to " + want);
      } else if(rec && rec.sha256 === llmsSha){
        fail("the sitemap dates llms.txt " + llmsLast + " but the file has not changed since " + rec.lastmod +
             " (qa/llms-txt.json) — a lastmod that moves without the body is the freshness signal lying the other way");
      } else {
        fail("llms.txt changed and the sitemap still dates it " + llmsLast + " — the newest CHANGELOG release is " + rel +
             ", and a crawler is told the one file written for it is stale. Fix with: npm run bless");
      }
    }
    if(BLESS && (!rec || rec.sha256 !== llmsSha)){
      fs.writeFileSync(LLMS_REC, JSON.stringify({note: "Blessed by guard 67: llms.txt's hash and the release that last changed it. " +
        "A different hash on disk means the sitemap's llms.txt lastmod must carry the newest CHANGELOG date.",
        sha256: llmsSha, lastmod: want}, null, 2) + "\n");
      note("recorded llms.txt's hash against " + want + " in qa/llms-txt.json");
    }
  }
  note("page date " + rel + ", agreed in sitemap.xml, the JSON-LD, CHANGELOG.md and the BUILT line; llms.txt dated " + (llmsLast || "?"));
})();
...
(function(){
  ...
  VOCAB.forEach(function(pair){
    ...
    var box = {
      ...
      revealHero: function(){}, clearHash: function(){ box.cleared = true; },
      isPath: function(){ return false; }
    };
    vm.runInNewContext(src + "\nrouteHash();", box);
    /* 4.9.0: a view hash is consumed once it has routed. It used to stay in
       the address bar, so a reload put a reader who had since chosen another
       ordering back into "Viewing Bruce's life". */
    if(!box.cleared){
      fail("#" + tok + " routed but was left in the address bar — a view link " +
           "is consumed once, or a reload re-applies it over a later choice");
    }
  ...
  var box2 = {
    ...
    importCode: function(){ return null; }, revealHero: function(){}, clearHash: function(){},
    isPath: function(){ return false; }
  };
  vm.runInNewContext(src + "\nrouteHash();", box2);
  ...
  var box3 = {
    ...
    importCode: function(){ return null; }, clearHash: function(){},
    isPath: function(){ return false; }
  };
...
(function(){
  ...
  if(rows.length < 2){ fail("fewer than two dated releases to compare — the CHANGELOG has lost its history"); return; }
...
(function(){
  ...
  /* Every key shape Home can now emit has to route somewhere. 4.9.0: the
     key scheme lives in one table (GROUPINGS), and goToGroup() asks it —
     so the check is that the table carries every prefix and the jump reads
     the mode off it rather than re-inferring it. */
  var go = fn("goToGroup");
  if(!/groupingOfKey\(jk\)\.mode/.test(go)){
    fail("goToGroup() does not read the ordering off GROUPINGS — a key shape " +
         "the table knows could land in the wrong ordering");
  }
  var tbl = topVar("GROUPINGS"), prefixes = [];
  if(tbl && tbl.type === "ArrayExpression"){
    tbl.elements.forEach(function(el){
      var pf = objProp(el, "prefix"), md = objProp(el, "mode");
      if(pf && md) prefixes.push(pf.value + ":" + md.value);
    });
  }
  ["c:continuity", "e:life", "d:release"].forEach(function(k){
    if(prefixes.indexOf(k) < 0){
      fail("GROUPINGS has no row for the \"" + k.split(":")[0] + "\" key — Home can " +
           "emit one, and it would land in the wrong ordering");
    }
  });
...
(function(){
  ...
  BUILDERS.forEach(function(name){
    var src;
    if(!hasFn(name)) return fail("guard 79: no " + name + "() to check");
    src = fn(name);
...
(function(){
  ...
  /* THE TRACK AND THE ARC ARE ONE RING. Everything above reads #ringArc, so a
     ...
     another, and every assertion here would still pass. 3.0.0 moved both from
  ...
  /* THE RING MAY NOT SHRINK PAST ITS OWN READOUT. 3.0.0 brought the ring down
...
(function(){
  ...
  /* 4.9.0: the continuity row of GROUPINGS carries tag:eraTag, and
     modeGroups() calls it with the raw PATH group (gr.tag(g)) — never with
     the filtered film list. */
  var tbl = topVar("GROUPINGS"), tagged = false;
  if(tbl && tbl.type === "ArrayExpression"){
    tbl.elements.forEach(function(el){
      var md = objProp(el, "mode"), tg = objProp(el, "tag");
      if(md && md.value === "continuity" && tg && srcOf(tg) === "eraTag") tagged = true;
    });
  }
  if(!tagged){
  ...
  src = fn("modeGroups");
  var calls = src.match(/gr\.tag\(([^)]*)\)/g) || [];
  if(calls.length !== 1) fail("modeGroups() tags a group " + calls.length + " time(s), expected once");
  calls.forEach(function(c){
    var arg = c.slice(7, -1).trim();
    if(!/^[A-Za-z_$][\w$]*$/.test(arg) || arg === "fs"){
      fail("modeGroups() calls " + c + " — eraTag() takes the unfiltered group, " +
...
(function(){
  ...
  /* The distribution the findings doc landed on, rechecks resolved: 93 MPA
     ratings + 27 honest no-rating-exists NR + 81 TV-system = 201 (5.3.1: 94 +
     26 + 81), and the five
     ...
     its strictest hour's broadcast rating.) 5.3.1, from the 5.3.0 audit,
     twelve values moved with a source each: Harley Quinn's Valentine's
     special TV-14 → TV-MA (Plex, HBO Max, JustWatch agree); Teen Titans: The
     Judas Contract NR → PG-13 and Reign of the Supermen NR → PG-13 (MPA
     certificates on Rotten Tomatoes — NR means none was ever issued, and
     both were); Batman & Mr. Freeze: SubZero PG → NR (no certificate
     citable: ParentPreviews, AllMovie and Common Sense all list it Not
     Rated); and one source per show for the four shows whose seasons had
...
(function(){
  ...
  if(BLESS){
    o["@graph"] = graphOrder(graph.filter(function(n){ return n["@type"] !== "ItemList"; }).concat([want]));
...
(function(){
  ...
  if(/beltOpen/.test(fn("persistNow")) || schemaKeys().indexOf("beltOpen") >= 0){
  ...
  /* 2.5.0 soak fix: the entry animation plays once, on the closed\u2192open
  ...
  if(!/\.includes\.opening \.scope\{animation:pouch/.test(HTML)){
    fail("the pouch entry animation lost its .opening scope \u2014 either it " +
         "animates on every render again (the 2.5.0 soak note, three times " +
...
(function(){
  ...
  ["counts\\(\\)", "pool\\(\\)", "modeGroups\\("].forEach(function(fnName){
  ...
  /* 5.3.1: `.homefoot a` left the selector list — the footer holds no
     anchor (the source link's seat is the Progress build line, a .note). */
  if(!/\.note a\{[^}]*text-decoration:underline/.test(HTML)){
...
(function(){
  ...
  /* 3: the ratings legend line sat misaligned among the legend entries —
     then (2.5.0 soak) its flex layout dropped the whole sentence below the
  ...
  if(!/class="rleg"/.test(HTML) || !/\.legend \.rleg\{[^}]*display:block/.test(HTML)){
    fail("the ratings legend line lost its inline flow \u2014 the sentence drops " +
         "below the badges as one block again, the 2.5.0 soak note");
  }
...
(function(){
  ...
  if(BLESS){
    o["@graph"] = graphOrder(graph.filter(function(n){ return n["@type"] !== "FAQPage"; }).concat([want]));
    blessHtml(HTML.replace(ldm[1], JSON.stringify(o)));
...
(function(){
  ...
  /* 5.3.1: the surgery is patchRow(v, id) — extracted so a tick from
  ...
  /* 5.3.1: the tally is computed once and handed to both helpers
     (gSub(g, d, k) / gBarFill(g, d, k)); the helpers are still the seat. */
  if(gb && (!/gSub\(g(?:, d, k)?\)/.test(gb) || !/gBarFill\(g(?:, d, k)?\)/.test(gb))){
...
(function(){
  ...
  /* 4.9.2, EARLY HINTS: six rel=preload font hints under /. With the zone
     toggle on (Speed → Optimization → Protocol — a panel step, recorded in
     RELEASING's wire checks because no guard can read a panel), Cloudflare
     caches these from the document's response and answers the next request
     with a 103 carrying them, so the font fetches overlap the HTML
     transfer. Three-way set equality: the Link hints, the <head>'s own
     <link rel="preload"> tags, and docs/fonts/*.woff2 must all name the
     same six files — a face that joins or leaves moves all three in one
     commit, and a hint pointing at a file that is not preloaded in the
     head (or not on disk) is a 103 that warms nothing. Token form
     (rel=preload, unquoted) pinned: the hints are consumed by the 103
     machinery, not by an HTML parser. */
  (function(){
    var rootBlock = block104("/");
    var hintRe = /^\s+Link:\s*<\/fonts\/([a-z0-9.\-]+\.woff2)>;\s*rel=preload;\s*as=font;\s*crossorigin\s*$/gm;
    var hints = [], hm;
    while((hm = hintRe.exec(rootBlock))) hints.push(hm[1]);
    var headPre = [], pm2, preRe2 = /<link rel="preload"[^>]*href="fonts\/([a-z0-9.\-]+\.woff2)"/g;
    var head104 = HTML.slice(0, HTML.indexOf("</head>"));
    while((pm2 = preRe2.exec(head104))) headPre.push(pm2[1]);
    var onDisk104 = fs.readdirSync(path.join(PUBLIC, "fonts"))
      .filter(function(f){ return /\.woff2$/.test(f); });
    var sets = [["the / rule's Link hints", hints.slice().sort()],
                ["the head's preload tags", headPre.slice().sort()],
                ["docs/fonts/", onDisk104.slice().sort()]];
    for(var si = 1; si < sets.length; si++){
      if(sets[0][1].join(",") !== sets[si][1].join(",")){
        fail("docs/_headers' Early Hints font set does not match " + sets[si][0] +
             " — hints: [" + sets[0][1].join(", ") + "], " + sets[si][0] + ": [" +
             sets[si][1].join(", ") + "]. The three sets move together or a 103 " +
             "warms the wrong files");
        return;
      }
    }
    if(hints.length !== 6){
    ...
    if(/rel=preload/.test(block104("/*"))){
      fail("a rel=preload hint is declared under /* — a hint describes the " +
           "document; /* would attach it to the fonts and icons themselves");
    }
  })();
    ...
    LOC105.forEach(function(u){
      if(/\/orders\.txt$/.test(u)){
        fail("orders.txt is submitted in sitemap.xml as " + u + " — it carries " +
             "the same 205 entries as the crawlable seed, so indexing both asks " +
...
/* The faces are subset to Latin-1 plus punctuation, and THE FAILURE MODE A
   ...
   the record of what they contain can only move together. Limelight shipped
   whole until 5.3.1 because its OFL header reserves the name and a Modified
   Version may not carry it; it ships subset and renamed "NW Deco" now, the
   answer 4.5.3 gave the Plex faces. (The rest is in NOTES.md.) */
...
(function(){
  ...
  /* 4.5.3: A SUBSET IS A MODIFIED VERSION, AND A MODIFIED VERSION MAY NOT
     ...
     is why Limelight shipped whole until 5.3.1 and why the subsets are NW
     Deco, NW Sans and NW Mono. The name table is read out of the woff2 here: the directory,
...
(function(){
  ...
  Object.keys(NESTED).forEach(function(n){
    var host = NESTED[n];
    /* Through sections(), like every other reader of the headers (4.9.0 —
       this was the one site that re-found them with its own regex), and
       through stripComments(), so a fail() quoted in prose cannot stand in
       for the real one. */
    var hostAt = -1, nAt = -1;
    sections().forEach(function(s){
      if(String(s.n) === host) hostAt = s.at;
      if(String(s.n) === n) nAt = s.at;
    });
    if(hostAt < 0 || nAt < 0 || nAt < hostAt){ fail("cannot locate sections " + host + " and " + n); return; }
    if(stripComments(SRC.slice(hostAt, nAt)).indexOf("fail(") < 0){
...
(function(){
  /* 4.9.0: render() no longer names either box. fieldSnap() records every
     input and textarea with an id in the panel (value, caret, focus) and
     fieldRestore() puts them back; FIELDKEEP names the boxes whose VALUE
     lives only in the DOM (#restorebox — #q's value comes from S.q). A third
     input needs no third copy of the dance, which was the 4.8.0 report's
     item 7.3. */
  var r = fn("render");
  ...
  if(!fnCalls("render", "fieldSnap") || !fnCalls("render", "fieldRestore")){
  ...
  var keep = topVar("FIELDKEEP");
  if(!keep || !objProp(keep, "restorebox")){
    fail("render() finds #restorebox but does not carry its value across — " +
         "FIELDKEEP no longer names it, so preserving the element is not " +
         "preserving the paste");
  }
  if(!/input\[id\],textarea\[id\]/.test(fn("fieldSnap"))){
    fail("render() no longer preserves the search box either — fieldSnap() " +
         "must walk every input and textarea, #q and #restorebox included");
  }
  ...
  if(!/fieldSnap\(p\)/.test(fpr) || !/fieldRestore\(p, fields\)/.test(fpr)){
...
(function(){
  ...
  suites.forEach(function(f){
    /* 5.3.1: the weight (guards fixtures + 40 × smoke fixtures) comes from
       the one census run-all.sh dispatches by, so the shards and the
       dispatch order can never disagree about what a suite costs. */
    var w = cenW[f] || 0;
...
(function(){
  ...
  /* ---- ORDER, which is the thing the counts above cannot see ----------
     ...
     one. */
  var rbody = hasFn("render") ? fn("render") : "";
  if(!rbody){
  ...
  /* 5.3.1: THE FIRST RENDER'S READ FORCES THE INITIAL LAYOUT, WHICH THE ORDER
     CLAUSE ABOVE CANNOT SEE. At boot the document still holds the crawler
     seed (431 elements the app never shows); render() opens with
     scrollKeep(), which lays the seed out — measured at 55 ms of first
     Layout at 4× throttle — and only then does buildDeck() replace it.
     Building the deck BEFORE the boot render lays out four empty panels
     instead (4 ms; boot 231 → 160 ms). render()'s own `sw` check keeps
     "built once" true. Order of calls, the shape of the clause above: in the
     boot block, buildDeck( precedes the first render(. */
  var boot = sliceOr("try{ routeHash(); }catch(e){}", "\nfunction splashOff(){");
  var bAt = boot.indexOf("buildDeck("), rAt = boot.indexOf("render();");
  if(bAt < 0 || rAt < 0 || bAt > rAt){
    fail("the boot render runs before buildDeck() — render() opens by reading the scroll position, which lays out the " +
         "431-element crawler seed the deck is about to throw away (55 ms of first Layout at 4× throttle, on the path to " +
         "first paint). Build the deck first; render()'s own .sw check keeps it built once");
  }
  note("layout reads: " + REFUSED.length + " properties refused outright, " +
       pins + " pinned read(s) across " + PINNED.length + " named sites, scroll " +
       "read before the first write, the deck built before the boot render");
})();
...
(function(){
  ...
  if(back.indexOf("preventScroll") < 0){
  ...
  if(snap.indexOf("document.activeElement") < 0){
  ...
  if(rest.indexOf("focusBack(") < 0 || rest.indexOf("attrEsc(") < 0){
...
(function(){
  /* 4.9.0: restore() is driven by the SCHEMA table — one row per persisted
     key, each with its own reader — and the store is synchronous (the
     window.storage adapter that made it Promise-shaped left with the 4.8.0
     report). The three containers are pinned by their rows' readers. */
  var rbody = optionalFn("restore", "nothing reads the saved state back");
  var rows = {watched: schemaRow("watched"), skipped: schemaRow("skipped"), rated: schemaRow("rated")};
  var bare = Object.keys(rows).filter(function(k){ return rows[k] && /\|\|/.test(rows[k].read); });
  if(bare.length){
    fail("restore() takes a progress container straight off the parsed payload " +
         "again (" + bare.join(", ") + "). A stored \"watched\":\"oops\" survives " +
         "the || and every toggleWatched throws on it — marking is dead until the reader resets");
  }
  if(!rows.watched || rows.watched.read !== "marksOf" ||
     !rows.skipped || rows.skipped.read !== "marksOf" ||
     !rows.rated || rows.rated.read !== "ratingsOf"){
  ...
  if(typeof box.validTs === "function"){
    [null, "", true, false, {}, [], NaN, Infinity, "later", undefined, 0, -1, "0"].forEach(function(bad){
    ...
    [1, 1754700000000, "1754700000000"].forEach(function(ok){
  ...
  if(!/try\{ raw = store\.get\(KEY\); \}\s*catch\(e\)\{ readFailed = true; canSave = false; return; \}/.test(rbody)){
    fail("restore()'s failed read no longer stops the writes. The failure " +
         ...
         "that failed means the stored state is unknown");
  }
  if((rbody.match(/readFailed = true/g) || []).length !== 2){
    fail("restore() no longer latches readFailed on both of its failure " +
         "paths — a read that throws and a body that reads but does not parse " +
         "are the same event: the stored state is unknown, so the writes stop");
  }
  ...
  ["persist", "persistNow"].forEach(function(name){
    var b = optionalFn(name, "nothing writes the saved state");
    if(!/if\(!store \|\| readFailed\) return;/.test(b)){
      fail(name + "() writes without asking whether the read succeeded");
    }
  });
  var pn = optionalFn("persistNow", "nothing writes the saved state");
  ...
  if(/catch\(e\)\{ canSave = false/.test(pn)){
    fail("persistNow() latches canSave on a failed write with no path back — " +
...
(function(){
  ...
  /* (Q1) The peek lights from S.mode, never from S.path. 4.9.0: the live
     setter is renderHead() — the .pathseg used to carry a data-lit of its
     own that no selector read (the 4.8.0 report's 2.2), and the guard pinned
     the vestige instead of the setter. */
  var rh = optionalFn("renderHead", "nothing lights the peek");
  if(!/getElementById\("beltpeek"\)\.setAttribute\("data-lit", S\.mode\)/.test(rh)){
  ...
  if(/"data-lit", S\.path/.test(rh) || /data-lit=/.test(mc)){
  ...
  if(!/segButtons\("path", PATHS, S\.path\)/.test(mc)){
  ...
  /* 4.9.0: the observer is part of the floor (every browser with `inert`,
     which the deck relies on, has it), so the detection went; parkFocus()
     stays the first thing beltWatch() does, unconditionally. */
  if(!/^function beltWatch\(\)\{\s*var root = document\.documentElement;\s*parkFocus\(\);/.test(bw) ||
     /"IntersectionObserver" in window/.test(bw)){
    fail("beltWatch() gates parkFocus() behind IntersectionObserver — the " +
         "observer is below the floor the script already requires (NOTES.md, " +
         "\"The floor\"), and the park toggle runs first, unconditionally, " +
         "because data-park renders from state");
  }
...
(function(){
  ...
  if(/beltDrop/.test(fn("persistNow")) || schemaKeys().indexOf("beltDrop") >= 0){
...
(function(){
  ...
  if(!/S\.beltDropping = true; render\(\); S\.beltDropping = false;/.test(fn("beltDropOpen")) ||
    ...
    fail("the entrance is not scoped to the one render that drops — the " +
         ...
         "which is the 2.5.0 soak note wearing the drop's clothes");
  }
  ...
  if(/animation:/.test(stripBase)){
    fail("the base .pathseg rule carries an animation — every render with a " +
         "dropped belt replays the entrance (2.5.0); only the data-toast " +
         "render animates");
  }
  ...
  if(/beltDropping/.test(fn("persistNow")) || schemaKeys().indexOf("beltDropping") >= 0){
...
(function(){
  ...
  var irow = schemaRow("insOff") || {};
  if(!/S\.insOff \? true : undefined/.test(irow.get || "")){
  ...
  if(!/v === true \? true : undefined/.test(irow.read || "")){
...
(function(){
  ...
  /* 5.3.1: an unchanged response is not rewritten. The cached entry and the
     network response carry the same ETag → no delete, no put; a different
     ETag → the write happens as before. The first drive above filled the
     store with a headerless response, which must still be replaced. */
  var etagRes = function(tag){
    return { ok: true, status: 200, clone: function(){ return this; },
             headers: { get: function(n){ return n.toLowerCase() === "etag" ? tag : null; } } };
  };
  putsBefore = puts.length;
  drive({ url: APP, method: "GET", mode: "" }, function(){ return SyncP.wrap(etagRes('"v1"')); });
  drive({ url: APP, method: "GET", mode: "" }, function(){ return SyncP.wrap(etagRes('"v1"')); });
  if(puts.length !== putsBefore + 1){
    fail("a response whose ETag matches the cached entry's was written again (" + (puts.length - putsBefore) +
         " put(s) for two fetches of one unchanged document) — every load rewrites the runtime cache for nothing");
  }
...
(function(){
  ...
  function req(accept, method, urlStr, extra){
    return { url: urlStr || "https://nightwatcher.life/", method: method || "GET",
             headers: { get: function(n){
               if(/accept/i.test(n)) return accept;
               return (extra && extra[n]) || null;
             } } };
  }

  /* 4.9.0: THE VALIDATORS RIDE THROUGH. The markdown response is no-cache,
     which means "revalidate before use" — and a response with no ETag and
     no Last-Modified cannot be revalidated, so every reload was a full
     fetch. The asset's own validators are copied onto the built response,
     an If-None-Match that matches answers 304 with no body, and HEAD never
     reads the body at all (the 4.8.0 report's 2.5). Driven with an assets
     mock that carries headers, beside the bare one above. */
  var textReads = 0;
  var envTagged = { ASSETS: { fetch: function(rq){
    var u = typeof rq === "string" ? rq : rq.url;
    if(/\/llms\.txt$/.test(u)) return new SyncV({ status: 200,
      headers: { get: function(n){ return n === "ETag" ? '"abc123"' : n === "Last-Modified" ? "Tue, 26 Aug 2026 00:00:00 GMT" : null; } },
      text: function(){ textReads++; return new SyncV(LLMS); } });
    return new SyncV(htmlRes);
  } } };
  var tagged = un(mod.fetch(req("text/markdown"), envTagged));
  var th = (tagged && tagged.init && tagged.init.headers) || {};
  if(th["ETag"] !== '"abc123"' || th["Last-Modified"] !== "Tue, 26 Aug 2026 00:00:00 GMT"){
    fail("the markdown response drops the asset's validators — under " +
         "Cache-Control: no-cache a response with no ETag or Last-Modified " +
         "cannot be revalidated, so every reload is a full fetch");
  }
  var notMod = un(mod.fetch(req("text/markdown", "GET", null, {"If-None-Match": '"abc123"'}), envTagged));
  ...
  if(!headTagged || headTagged.body !== null || textReads !== 0){
    fail("HEAD / (markdown) read the asset's body (" + textReads + " read(s)) — " +
         "HEAD answers the GET's headers, and reading a body to return null " +
  ...
  /* 4.9.2: narrowed to the DOCUMENT relations. The root's _headers block
     now also carries six rel=preload font hints for Early Hints, and those
     describe the HTML representation — a hint to preload display fonts on
     a markdown body would be noise a text client pays for. The built
     responses carry sitemap, canonical and describedby, exactly. */
  var rootDocLinks = (root["Link"] || "").split(", ")
    .filter(function(l){ return !/rel=preload/.test(l); }).join(", ");
  [["the markdown response", md], ["the markdown HEAD response", mdHead]].forEach(function(pair){
    var res = pair[1];
    if(!res || res === htmlRes || typeof res.then === "function") return;
    var got = (res.init.headers || {})["Link"] || "";
    if(got !== rootDocLinks){
      fail(pair[0] + "'s Link header is not the root's three document Link " +
    ...
    if(/rel=preload/.test(got)){
      fail(pair[0] + " carries a font preload hint — the hints describe the " +
...
(function(){
  ...
  if(!(/isFinite\(n\)/.test(co) && /n > 0/.test(co))){
...
(function(){
  var fsv = optionalFn("flagSave", "nothing shows the storage banner");
  if(fsv.indexOf('removeProperty("--hdrh")') < 0){
  ...
  var dl = optionalFn("dedupeLog", "nothing shapes the saved log");
...
/* README.md, NOTES.md, index.html's header block and this file's own opening
   ...
   negative-tested. A full mapping of the 670 fixtures onto the sections they
...
(function(){
  ...
  /* THE SECT RATCHET. run_case's sixth argument pins a fixture's match to its
     ...
     green_case IS NOT HARVESTED: it asserts an exit code, not a string, so
  ...
  fixtureCensus().suites.forEach(function(su){
    su.cases.forEach(function(c){
      ...
      expects.push({e: c.expect, sect: c.sect});
      if(c.suite !== "guards") return;
      if(!c.sect) noSect++;
    });
  });
  ...
  /* 5.3.1: A FIXTURE CREDITS ONE SECTION. Until now every section whose
     failure text contained the expect was marked covered, whatever the
     fixture's sect — so §33 ("One tagline, everywhere") read as covered by
     two §116 glyph fixtures whose expect, "does not carry", is a substring
     of §33's "does not carry the tagline", and had never had a fixture of
     its own; 21 fixtures were crediting 51 sections between them. A pinned
     fixture now credits its own section only (or nothing, if the phrase is
...
(function(){
  ...
  /* 5.3.1: the note reads the same clock the check did — under NW_TODAY it
     used to print the wall-clock distance beside a pinned verdict. */
  var noteToday = (process.env.NW_TODAY && !process.env.CI && /^\d{4}-\d{2}-\d{2}$/.test(process.env.NW_TODAY))
                  ? Date.parse(process.env.NW_TODAY) : Date.now();
  note("security.txt: URL contact, policy at SECURITY.md, expires in " +
       (exp ? Math.floor((Date.parse(exp) - noteToday) / 86400000) : "?") +
       " days" + (noteToday !== Date.now() && process.env.NW_TODAY ? " from NW_TODAY" : "") + ", no-cache declared");
})();
...
(function(){
  ...
  /* 4.9.0: the comparators sit on the GROUPINGS rows (sort:lifeCmp,
     sort:releaseCmp) and modeGroups() applies whichever the row names —
     buildGroups() takes its groups from there. */
  var sorts = {};
  var tbl = topVar("GROUPINGS");
  if(tbl && tbl.type === "ArrayExpression"){
    tbl.elements.forEach(function(el){
      var md = objProp(el, "mode"), so = objProp(el, "sort");
      if(md && so) sorts[md.value] = srcOf(so);
    });
  }
  var mg = optionalFn("modeGroups", "nothing builds the groups");
  if(sorts.life !== "lifeCmp" || !/fs\.sort\(gr\.sort\)/.test(mg) || !/modeGroups\(S\.mode\)/.test(bg)){
  ...
  if(/\.sort\(function/.test(bg) || /\.sort\(function/.test(mg) || /sort:function/.test(srcOf(tbl))){
  ...
  /* 4.9.0: the same-year tie is broken by the catalogue's own order (group,
     then position), not by title — a title tie-break sorted Crisis on
     Infinite Earths Part Three before Part Two and Batman Beyond season one
     before its pilot feature (the 4.8.0 report's 1.1). */
  if(rel.indexOf(".y") < 0 || rel.indexOf("gi") < 0 || rel.indexOf("ix") < 0 || /localeCompare/.test(rel)){
    fail("releaseCmp no longer keys on the year, then the catalogue's own " +
         "order (gi, ix) — a title tie-break inverts numbered parts, and " +
         "section 105 publishes the result as release order");
  }
...
(function(){
  ...
  /* 5.3.1: the nudge counts TITLES, not clocks. A skip stamps its own clock
     and, through unmarkWatched(), a watched tombstone — the merge's "skip
     beats a stale tick" signal — so counting clock entries told a reader
     who skipped once that they were "behind by 2 changes". marksSince()
     counts distinct ids across the three clocks; the tombstone stays. */
  var ms = {S:{clk:{w:{a:10, b:10}, s:{a:10}, r:{b:10, c:1}}}, HAS:Object.prototype.hasOwnProperty};
  new vm.Script(fn("marksSince")).runInContext(vm.createContext(ms));
  if(ms.marksSince(5) !== 2){
    fail("marksSince(5) counts " + ms.marksSince(5) + " over clocks {w:a,b s:a r:b,c(stale)} — the backup nudge counts " +
         "clock entries, so a skip (its own clock plus the watched tombstone) reads as two changes; count titles");
  }
  note("backups: " + stamps.length + " stamp sites, every one gated on a real " +
       "write; the handle helpers resolve booleans and stamp nothing; the nudge counts titles");
})();
...
(function(){
  ...
  /* 5.3.1: render() takes an option — {quiet:true} for the six one-view
     actions (a search keystroke, a peek, a fold, the code, the nag, the
     reset arm each touch exactly one view, and used to rebuild The Path
  ...
  /* 5.3.1: a search keystroke is the deck's most frequent action and used
     to rebuild The Path whole — innerHTML of the panel, then the caret put
     back into an input the render had destroyed (9–37 ms sync plus a
     105–133 ms task per debounced keystroke at 4× throttle). The handler
     goes through searchApply(), which toggles hidden on rows and groups in
     place; the identity phase holds it byte-identical to a full render. */
  var qh = null;
  walk(FN_AST.body, function(n){
    if(n.type === "CallExpression" && calleeName(n.callee) === "?.addEventListener" &&
       n.arguments[0] && n.arguments[0].value === "input" && n.arguments[1]) qh = n.arguments[1];
  });
  var qCalls = qh ? callsIn(qh) : [];
  ...
  /* The other doors into a tab snap too — a hash arrives with the deck at
     whatever panel it was on. Counted, not merely found: there are exactly
     three (boot restore, hashchange, and goToGroup's jump since 5.3.0), and
     losing any one leaves a door rendering a tab into a deck still showing
     the old one. */
...
(function(){
  ...
  try{ listed = gitIndexPaths(raw); }catch(e){ listed = null; }
  if(!listed){
...
(function(){
  ...
  /* 4.9.0: the codes live on the GROUPINGS rows (codeOf), and both
     renderers take their groups from modeGroups(), which carries code on
     every group it returns. */
  var codes = {};
  var tbl = topVar("GROUPINGS");
  if(tbl && tbl.type === "ArrayExpression"){
    tbl.elements.forEach(function(el){
      var md = objProp(el, "mode"), co = objProp(el, "codeOf");
      if(md && co) codes[md.value] = srcOf(co).replace(/\s+/g, " ");
    });
  }
  if(!/"u" \+ g\.n/.test(codes.continuity || "") || !/"e" \+ g\.k/.test(codes.life || "") ||
     !/"d" \+ g\.k/.test(codes.release || "") || !/code:gr\.codeOf\(g\)/.test(fn("modeGroups"))){
  ...
  if(!/modeGroups\(pm\)/.test(card) || !/roofOf\(g\.code, fs\.length\)/.test(card) ||
     !/modeGroups\("continuity"\)/.test(vs)){
...
(function(){
  ...
  if(/#splash\{[^}]*animation:(?!splashfail)/.test(HTML) || /#splash svg\{[^}]*animation/.test(HTML)){
  ...
  /* 4.9.0: THE FAILURE EXIT. splashOff() ran only from the boot; anything
     thrown before it — a CSP hash mismatch from a whitespace-altering proxy
     (no script runs at all), a throw in a listener registration, a throw
     inside the first render — left a permanent bat over the page whose
     seed was written for exactly that reader. Three doors now: a CSS
     failsafe that fades the cover after 5 s whether or not any script ran
     (the only door that works when the CSP refuses the script), a window
     error listener registered at the top of the script, and try/finally
     around the boot render. */
  if(!/@keyframes splashfail\{to\{opacity:0;visibility:hidden;\}\}/.test(HTML) ||
     !/#splash\{[^}]*animation:splashfail \.3s ease 5s forwards;/.test(HTML)){
    fail("the splash has no CSS failsafe — when the CSP refuses the script " +
         "(a proxy that reflows whitespace breaks the hash) no code runs, and " +
         "the cover must come down on its own after a few seconds");
  }
  if(!/"use strict";\nwindow\.addEventListener\("error", function\(\)\{ splashOff\(\); \}\);/.test(HTML)){
    fail("the error listener that drops the splash is not the first statement " +
         "of the script — an error thrown before it is registered leaves the " +
         "cover up, so it goes before everything");
  }
  ...
  var boot = (HTML.match(/\nrestore\(\);\n[\s\S]*?\n\}\n/) || [""])[0];
  /* 5.3.1: the deck is built inside the same try, before the first render
     (section 120's boot clause says why); the finally is unchanged. */
  if(!/try\{\s*(?:buildDeck\([^;]*\);\s*)?render\(\);\s*snapTo\(S\.tab\);\s*\} finally \{\s*splashOff\(\);\s*\}/.test(boot)){
    fail("splashOff() is not called after the first render() in a finally — " +
         "the cover comes down before the app is under it, or never (a throw " +
         "inside the first render would leave it up)");
  }
...
/* 4.8.0. Until now the tier lived only in The path's chips, which hide rows
   ...
   chips stay — all eight since the safe chip (5.1.0), the owner's call — because a filter is a glance
...
(function(){
  ...
  /* The pouch. */
  /* 4.9.0: the four switches render through one segmented() helper (the
     4.8.0 report's 7.4), each from a named option table; the pouch's
     options are TIERS and its buttons carry data-tier because the helper
     is handed the attribute name. */
  var ts = optionalFn("tierSwitch", "the belt has no tier pouch");
  var tiers = srcOf(topVar("TIERS"));
  ["ess", "core", "all"].forEach(function(t){
    if(tiers.indexOf('"' + t + '"') < 0) fail("tierSwitch() offers no \"" + t + "\" button");
  });
  ...
  var fmts = srcOf(topVar("FORMATS"));
  if(!/\["all","Animated \+ live"\]/.test(fmts) || !/segmented\("scope fmt", "Format", "format", FORMATS, S\.format\)/.test(fs)){
  ...
  [tiers, fmts, srcOf(topVar("SCOPES")), optionalFn("scopeSwitch")].forEach(function(src){
  ...
  ["ess", "core", "opt"].forEach(function(k){
    if(cs.indexOf('"' + k + '"') < 0){
      fail("the " + k + " chip left The path — the chips stay, all eight: a " +
...
/* ---------- 153. The head's required tag set, by name ---------------- */
/* 4.9.0. The 4.8.0 report deleted each of the 42 <head> tags in turn and ran
   npm test: thirteen went with a green run — charset, the manifest link
   (the one that makes the PWA installable; the manifest FILE was guarded,
   the link was not), the description, og:url, og:description, og:type,
   og:locale, og:image:alt and the four home-screen metas. Every other
   section here checks a tag's VALUE once it has found it; none asked
   whether the tag was there, so a missing tag switched its check off. This
   is the section-65 allowlist pattern turned around: the head's required
   set, each tag named, each one's absence a failure of its own. A tag that
   joins the head joins this list in the same commit, or the next deletion
   sweep finds it. (The sweep itself is qa/negative/negtest610.sh.) */

(function(){
  var head = HTML.slice(0, HTML.indexOf("</head>"));
  var REQUIRED = [
    ["<meta charset>",                   /<meta charset="utf-8">/],
    ["the viewport meta",                /<meta name="viewport" content="width=device-width, initial-scale=1, viewport-fit=cover">/],
    ...
    ["<title>",                          /<title>[^<]+<\/title>/],
    ["the description meta",             /<meta name="description" content="[^"]+">/],
    ["og:type",                          /<meta property="og:type" content="website">/],
    ["og:title",                         /<meta property="og:title" content="[^"]+">/],
    ["og:description",                   /<meta property="og:description" content="[^"]+">/],
    ["og:url",                           /<meta property="og:url" content="https:\/\/nightwatcher\.life\/">/],
    ["og:site_name",                     /<meta property="og:site_name" content="Night Watcher">/],
    ["the canonical link",               /<link rel="canonical" href="https:\/\/nightwatcher\.life\/">/],
    ...
    ["the .ico favicon link",            /<link rel="icon" href="favicon\.ico" sizes="any">/],
    ["the SVG icon link",                /<link rel="icon" type="image\/svg\+xml" href="icon\.svg">/],
    ["the orders.txt alternate link",    /<link rel="alternate" type="text\/plain" href="orders\.txt"/],
    ["the 16px icon link",               /<link rel="icon" type="image\/png" sizes="16x16" href="favicon-16x16\.png">/],
    ["the 32px icon link",               /<link rel="icon" type="image\/png" sizes="32x32" href="favicon-32x32\.png">/],
    ["the 48px icon link",               /<link rel="icon" type="image\/png" sizes="48x48" href="favicon-48x48\.png">/],
    ["the 192px icon link",              /<link rel="icon" type="image\/png" sizes="192x192" href="icon-192\.png">/],
    ["the 512px icon link",              /<link rel="icon" type="image\/png" sizes="512x512" href="icon\.png">/],
    ["the apple-touch-icon link",        /<link rel="apple-touch-icon" href="apple-touch-icon\.png">/],
    ["theme-color",                      /<meta name="theme-color" content="#[0-9A-Fa-f]{6}">/],
    ["application-name",                 /<meta name="application-name" content="Night Watcher">/],
    ["msapplication-TileColor",          /<meta name="msapplication-TileColor" content="#[0-9A-Fa-f]{6}">/],
    ["msapplication-TileImage",          /<meta name="msapplication-TileImage" content="mstile-144x144\.png">/],
    ["the referrer meta",                /<meta name="referrer" content="strict-origin-when-cross-origin">/],
    ["the JSON-LD block",                /<script type="application\/ld\+json">/],
    ["the <noscript> splash undo",       /<noscript><style>#splash\{display:none;\}<\/style><\/noscript>/]
  ];
  var gone = REQUIRED.filter(function(t){ return !t[1].test(head); }).map(function(t){ return t[0]; });
  if(gone.length){
    fail("the head has lost " + gone.length + " required tag(s): " + gone.join(", ") +
...
(function(){
  ...
  new vm.Script([fn("isDone"), fn("isSkip"), fn("isParked"), fn("isParkedId"), fn("counts"),
                 fn("pickStands"), fn("upNext"), fn("behind")].join("\n"))
    .runInContext(vm.createContext(box));
  var c = box.counts();
  ...
  /* 5.3.1: the skipped loop alone — the rated loop after it carries its own
     gate now, which would otherwise satisfy this slice. */
  if(!/isParkedId\(id\)/.test(am.slice(am.indexOf("res.skipped"), am.indexOf("res.rated")))){
    fail("applyMarks() lets a pasted backup skip a parked title");
  }
  if(!/!S\.watched\[k\] && !isParkedId\(k\)\)\{ S\.skipped\[k\] = 1;/.test(fn("mergeTab"))) fail("the cross-tab merge lets another tab skip a parked title");
  ...
  if(!/if\(isParked\(f\)\) return parkedRow\(f(?:, hid)?\);/.test(fn("filmRow"))) fail("filmRow() draws a parked entry as an ordinary row");
  var pr = fn("parkedRow");
...
(function(){
  ...
  /* 5.3.1: upNext() is a pure read — the pick's validity lives in
     pickStands(), and its expiry in expirePick(), called at the three
     entry points (render, tickUpdate, revealHero) rather than hidden inside
     a getter every render path called. */
  var un = fn("upNext"), ps = fn("pickStands");
  if(!/pickStands\(\)/.test(un) || !/S\.pick/.test(ps) || !/!isDone\(pk\) && !isSkip\(pk\)/.test(ps)) fail("upNext() ignores the pick, or honours a pick that has since been ticked or skipped");
  if(/S\.pick = /.test(un)) fail("upNext() writes state again — the pick's expiry belongs to expirePick(), not to a getter every render path calls");
  ["render", "tickUpdate", "revealHero"].forEach(function(name){
    if(!fnCalls(name, "expirePick")) fail(name + "() no longer expires a stale pick on the way in — a ticked or skipped pick would linger in S.pick");
  });
...
(function(){
  ...
  var box = {};
  new vm.Script(fn("validTs") + "\n" + no).runInContext(vm.createContext(box));
  ...
  note("three zero-state features: up-to-here through markWatched(), nights, five stars — and what's left stays gone");
})();
...
(function(){
  ...
  var box = {};
  /* 5.3.1: the floor's constants come off the tree (topVar), not a restated
     literal — a restated literal is a second copy that can disagree. */
  new vm.Script("var DONEBY_NIGHTS = " + srcOf(topVar("DONEBY_NIGHTS")) + ", DONEBY_TITLES = " + srcOf(topVar("DONEBY_TITLES")) + ";\n" + db)
    .runInContext(vm.createContext(box));
  ...
  if(!/scoreboard\(c\) \+ nightsLine\(\) \+ doneByLine\(c\)/.test(fn("viewStats"))){
...
/* 5.1.1. 5.0.0 promised a parked title "cannot be ticked or skipped, from
   ...
   mark, requires each to sit behind an isParkedId test, and fails a seat
   nobody gated. The log is a door too.
   ...
   load, forever — so the sweeps run behind it, and the watched sweep
   takes the mark's log nights with it.

   5.3.1, from the 5.3.0 audit: the census read SPELLING. It matched
   `S.watched[x] = 1` in the shapes it had thought of, so a seat written
   as `S.watched[id] = S.watched[id] || 1`, an `Object.assign(S.watched,
   m)` or a helper handed the object walked past it while the note still
   printed "each gated" — verified with two planted doors. And the rated
   mark was never in its set at all: a rating on a parked title survived
   every seat, landed in "Your five stars" as a 2028 film and rode the
   backup code. The census is now an AST walk over the parsed program:
   every assignment (any operator) into S.watched / S.skipped / S.rated,
   every wholesale assignment to one, every call handed one as an
   argument, every alias of one. Each member write must be a listed door
   AND carry isParkedId(<its own index>) in the innermost loop or
...
(function(){
  ...
  var DOORS = [
    ["the tap (markWatched)",            "markWatched", "watched"],
    ["the tap (toggleSkip)",             "toggleSkip",  "skipped"],
    ["the tap (rate)",                   "rate",        "rated"],
    ["a pasted code or JSON (watched)",  "applyMarks",  "watched"],
    ["a pasted code or JSON (skipped)",  "applyMarks",  "skipped"],
    ["a pasted code or JSON (rated)",    "applyMarks",  "rated"],
    ["another tab (watched)",            "mergeTab",    "watched"],
    ["another tab (skipped)",            "mergeTab",    "skipped"],
    ["another tab (rated)",              "mergeTab",    "rated"]
  ];
  ...
  if(!fnIndex()){
    /* No parser (a bare local clone): the 5.3.0 spelling census stands in,
       with the rated seats added. CI never takes this branch (§fnIndex). */
    var writes = HTML.match(/S\s*(?:\.\s*(?:watched|skipped|rated)|\[\s*["'](?:watched|skipped|rated)["']\s*\])\s*\[[^\]\n]+\]\s*=[^=]/g) || [];
    if(writes.length !== DOORS.length){
      fail(writes.length + " statements write a mark and this section names " + DOORS.length +
           " — a new door opened without a gate, or one closed without leaving the list (legacy census: acorn is not installed)");
    }
  } else {
    function markOf(node){
      if(!node || node.type !== "MemberExpression") return null;
      if(node.object.type !== "Identifier" || node.object.name !== "S") return null;
      if(!node.computed) return MARKS[node.property.name] ? node.property.name : null;
      if(node.property.type === "Literal") return MARKS[node.property.value] ? node.property.value : null;
      return "<computed>";
    }
    ...
    function visit(node, fns, scope){
      ...
      if(node.type === "CallExpression"){
        ...
        node.arguments.forEach(function(a){
          var am = markOf(a);
          if(am && am !== "<computed>") handed.push({fn: outer, callee: callee, mark: am, src: srcOf(node)});
        });
      }
      Object.keys(node).forEach(function(k){
        if(k === "type" || k === "start" || k === "end") return;
    ...
    if(aliases.length){
      fail(aliases.length + " statement(s) alias S.watched, S.skipped or S.rated into another name (" + aliases[0].slice(0, 40) +
           ") — an alias is a door the census cannot see; write through the canonical object");
    }
    wholesale.forEach(function(w){
      var r = w.node.right;
      if(!(r.type === "ObjectExpression" && r.properties.length === 0)){
        fail(w.fn + "() assigns S." + w.mark + " wholesale from " + srcOf(r).slice(0, 40) +
             " — a wholesale write is a door with no gate; only an erase (= {}) may replace a mark object");
      }
    });
    ...
    handed.forEach(function(h){
      if(!READERS[h.callee]) fail(h.fn + "() hands S." + h.mark + " to " + h.callee + "() — a door the census cannot see through; " +
                                   "write through the canonical object in a listed door, or add a proven reader to this section");
    });
    /* Each member write is a listed door, gated in its own scope. */
    var found = DOORS.map(function(){ return 0; });
    seats.forEach(function(st){
      var di = -1;
      DOORS.forEach(function(d, i){ if(d[1] === st.fn && d[2] === st.mark) di = i; });
      if(di < 0){
        fail('"' + st.src.slice(0, 50) + '" in ' + st.fn + "() writes a " + st.mark + " mark and is not a listed door — " +
             "a new door opened without a gate, or the seat moved and this list must move with it");
        return;
      }
      found[di]++;
    ...
    DOORS.forEach(function(d, i){
      if(found[i] !== 1) fail("this section names " + d[0] + " as a door and the census finds " + found[i] +
                              " write(s) there — the seat moved, or one closed without leaving the list");
    });
    if(seats.length !== DOORS.length){
    ...
    var caught = false;
    walk(FN_AST.body, function(n){
      if(n.type !== "CallExpression" || calleeName(n.callee) !== "window.addEventListener") return;
      if(!n.arguments[0] || n.arguments[0].value !== "storage") return;
      walk(n.arguments[1], function(t){
        if(t.type !== "TryStatement" || !t.handler) return;
        var body = callsIn(t.block), hand = callsIn(t.handler);
        if(body.indexOf("mergeTab") >= 0 && hand.indexOf("persist") >= 0 && hand.indexOf("render") >= 0) caught = true;
      });
    });
    if(!caught) fail("the storage listener no longer merges inside a try — a throw mid-merge leaves the adopted state in RAM only, and the next tick persists a half-merged blob");
    /* Restore is a door: all three sweeps, by call, not by spelling. */
    ["dropParkedSkips", "dropParkedWatched", "dropParkedRated"].forEach(function(sw){
      if(!fnCalls("restore", sw)) fail("restore() no longer runs every sweep — " + sw + "() is not called — the door 5.1.1 left open (a parked mark re-adopted wholesale from storage) is open again");
    });
  }
...
(function(){
  ...
  /* 5.3.1, from the external 5.3.0 read: THE MARKS SURVIVED, THE STATES DID
     NOT. 5.3.0 painted the ornaments and left every state that a background
     alone carried — the pressed chip, the pressed belt and scope buttons,
     the current tab (signal ink only), the done tick, the lit belt-peek
     segment, the essentials segments, the skyline crown, the group bar's
     fill, the here-group's corner gradients — to wash out under the UA's
     High Contrast palette. The line drawn: the palette is the reader's (no
     forced-color-adjust:none on brand chrome — the wordmark, the path title,
     the belt, the group heads, prose all take system colours), and only a
     STATE is painted, in system colours: Highlight/HighlightText for a
     selected or done control, CanvasText for a fill, GrayText for a skipped
     one, an underline for the current tab, an outline for the here group.
...
/* 4.9.0: A WARNING IS NOT A PASS IN CI. Nine sites used to warn where the
   ...
   warning(s))" with exit 0, which is the silence-reads-as-success family
   section 15 names. Those sites fail now; what still warns is advice a
   local clone can act on (no snapshot yet, a parser missing, headroom
   running low). In CI, where nobody reads the log unless it is red, any
   warning at all is red. */
if(warns.length && process.env.CI){
  console.log("\n" + warns.length + " WARNING" + (warns.length > 1 ? "S" : "") +
              " IN CI — a warning is a check that could not run, and a check that " +
