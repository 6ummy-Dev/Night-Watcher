...
the week's Batman news (screen, comics, games, toys, books) and where each
story sits on the map, no spoilers, every source linked. No. 0 is about the map itself.

It is **not part of the app**. The service worker never caches it, and it
runs no script: static pages under a default-deny Content-Security-Policy of
their own (`docs/_headers`). The app's one door to it is Home's *Read the
...
| `docs/nocturne/` | Nocturne, the paper: every issue, its images, the archive, the RSS feed with its browser stylesheet `feed.css`, and `nocturne.css`. Written by `npm run nocturne:build`, never by hand; guard 163 holds it to the build |
...
trusted. That evidence lives in `qa/negative/` — 84 negative suites, 1502
