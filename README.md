...
- **A weight budget** — *guarded.* `docs/index.html` must stay under 250 KiB raw and 80 KiB gzipped; it is currently 245 KiB / 68 KiB — guard 29 measures `bytes / 1024`, so every weight figure here is KiB, and the same file read in decimal kB is 252 and looks like an overshoot it is not. The subset webfonts sit outside that budget and are held by their own manifest (`qa/font-subset.json`). Every raise of the ceiling is an owner's call recorded in the CHANGELOG, never a drift. A single file that opens instantly is the whole premise, and arithmetic is the only thing protecting it.
...
**Nocturne** is Night Watcher's paper, at [`/nocturne/`](https://nightwatcher.life/nocturne/)
(from 6.2.0). Its one edition, **the Night Final**, runs every Sunday, late:
the week's Batman screen news and what each story does to a watch order, no
spoilers, every source linked. No. 0 is about the map itself.

It is **not part of the app**. The service worker never caches it, the app
never links to it, and it runs no script: static pages under a
default-deny Content-Security-Policy of their own (`docs/_headers`). It is
reached through the sitemap, its RSS feed and the X post that announces
each issue.

An AI drafting agent writes each issue as markdown in `nocturne/issues/`,
following `nocturne/BRIEF.md` and `nocturne/VOICE.md`, and opens a pull
request; the owner reviews and merges every one. `npm run nocturne:build`
...
committed, and guards 163–168 hold it byte for byte to what the build
...
| `docs/manifest.json` | Web app manifest — lets Android and desktop install it with the right name and icon, and shows two screenshots in the install dialog |
...
| `docs/sitemap.xml` | Two URLs — the single-page app and `llms.txt` — then Nocturne's block, which the paper's build writes |
| `docs/share.png` | The 1200×630 share card. Generated by `qa/make-share-card.mjs`; guard 91 holds it honest |
| `docs/shot-narrow.png` | The install dialog's phone screenshot — Home on a first visit, 780×1688. Generated by `qa/make-screenshots.mjs`; guard 160 holds it honest |
| `docs/shot-wide.png` | The install dialog's desktop screenshot — The Path in Bruce's life order, 1280×720. Same generator, same guard |
...
| `docs/nocturne/` | Nocturne, the paper: every issue, its images, the archive, the RSS feed and `nocturne.css`. Written by `npm run nocturne:build`, never by hand; guard 163 holds it to the build |
...
| `nocturne/` | Nocturne's source: `BRIEF.md` and `VOICE.md` (the owner's rules for the drafting agent, which never edits them) and `issues/`, one folder per issue — `issue.md` and its images |
| `qa/nocturne.js` | The paper's builder and checker (`npm run nocturne:build`, `npm run nocturne:check`). Reads the catalogue out of `docs/index.html`; guards 163–168 require the same file |
| `qa/nocturne-fixture/` | Two test issues (No. 0 and an invented No. 1) that guards 163–168 build and check on every run. Never published |
...
| `qa/make-screenshots.mjs` | Draws the two install screenshots from the served tree (seeded storage, no clicks, a DOM settle, quantized) and records them |
| `qa/screenshots.json` | What the screenshots were drawn from — the catalogue's three counts, each file's form factor, size, label and hash — held against the manifest and the data by guard 160 |
| `qa/aria/` | The accessibility-tree corpus: the header, live panel and tab bar a screen reader hears in eight states, written and diffed by `qa/browser-check.mjs` (`npm run browser -- --bless`); guard 161 holds its shape |
...
trusted. That evidence lives in `qa/negative/` — 84 negative suites, 1490
...
The second half of `npm test` is `qa/smoke.js`, a headless render test that boots the real page in jsdom and drives what static analysis can't reach: rendering, scope switching, hostile import, the backup parser against old, forward-dated, pasted and malformed codes, a copy with `localStorage` throwing, the cross-tab merge, and the path end to end — 520 checks. jsdom is a declared dev dependency; a local clone without it skips the suite and says so, CI treats the skip as a failure.
