...
- **A weight budget** — *guarded.* `docs/index.html` must stay under 165 KB raw and 50 KB gzipped; it is currently 161 KB / 48 KB. The raw ceiling moved 150 → 160 in 1.9.5 for the ratings data and 160 → 165 in 2.0.0 for the progress card, each raise an owner's call recorded in the CHANGELOG; the gzip budget has never moved. A single file that opens instantly is the whole premise, and arithmetic is the only thing protecting it.
...
trusted. That evidence lives in `qa/negative/` — 23 negative suites, 264
