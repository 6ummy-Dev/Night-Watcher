...
| `docs/nocturne/` | Nocturne, the paper: every issue, its images, the archive (The morgue), the RSS feed with its browser stylesheet `feed.css`, `nocturne.css`, the paper's own italic face and the `OFL.txt` that travels with it. Written by `npm run nocturne:build`, never by hand; guard 163 holds it to the build |
...
trusted. That evidence lives in `qa/negative/` — 85 negative suites, 1540
