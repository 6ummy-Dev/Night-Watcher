...
Built and maintained by one person, in the open: every change is written
down with its reason in `CHANGELOG.md`.
...
- **A weight budget** — *guarded.* `docs/index.html` must stay under 275 KiB raw and 80 KiB gzipped (raised from 250 in 6.3.0, the owner's call); it is currently 247 KiB / 68 KiB — guard 29 measures `bytes / 1024`, so every weight figure here is KiB, and the same file read in decimal kB is 253 and looks like an overshoot it is not. The subset webfonts sit outside that budget and are held by their own manifest (`qa/font-subset.json`). Every raise of the ceiling is an owner's call recorded in the CHANGELOG, never a drift. A single file that opens instantly is the whole premise, and arithmetic is the only thing protecting it.
...
the week's Batman news (screen, comics, games, toys, books), no spoilers,
every source linked. When a story touches a title on the map, a box says
where it sits; a strip under the banner, the Board, says what moved on the
map that week. No. 0 is the paper introducing itself, and its neighbour.
...
| `docs/nocturne/` | Nocturne, the paper: every issue, its images, the archive (The morgue), the RSS feed with its browser stylesheet `feed.css`, `nocturne.css`, `theme.js` (the paper's one script of its own), the paper's own italic face and the `OFL.txt` that travels with it. Written by `npm run nocturne:build`, never by hand; guard 163 holds it to the build |
...
| `.github/workflows/nocturne-fence.yml` | The fence around the drafting agent: a pull request by `nocturne-desk` may change `nocturne/issues/`, `docs/nocturne/` and `docs/sitemap.xml` only. Runs on `pull_request_target`, so the copy on `main` judges it; executes nothing from the pull request. Guard 163 holds its shape |
...
trusted. That evidence lives in `qa/negative/` — 86 negative suites, 1569
